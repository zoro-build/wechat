...
扫码授权 → 拉取任意公众号 → 导出归档（HTML / Markdown / Word / PDF）。图文与排版原样保留，离线随时可读。

[⬇️ 下载 Windows 桌面客户端][download]

公开下载 · 邀请制注册 · 当前支持 Windows，macOS 即将支持
...
公号三刀是一款**微信公众号文章下载桌面客户端**（当前支持 Windows，macOS 即将支持）。你只需用微信扫码授权一次，就能搜索任意公众号、把它的历史图文连同排版与图片/视频完整抓取到本地，然后离线阅读，或一键导出成 HTML / Markdown / Word / PDF 长期归档。
...
### 5. 多格式导出（HTML / Markdown / Word / PDF / ZIP）

把选中的文章导出为 **HTML / Markdown / Word（DOCX）/ PDF**，单篇或批量（批量自动打包成 ZIP）：
...
把关注的公众号生成 RSS 订阅源，在 Reeder / NetNewsWire 等任意阅读器里直接追更。可按订阅自定义拉取间隔，新文章自动入库。
...
| **03 抓取 / 导出** | 一键抓取图文正文，离线阅读，或批量导出 HTML / Markdown / Word / PDF 归档 |
...
> 各档价格与开通 / 续期方式请见 [站点说明](https://wechat.zoro.build/#plans) 或联系客服（暂未开放在线购买）。
...
**如何获得使用资格？**
安装包公开可下载，但使用需登录账号；当前注册为邀请制，前往[站点](https://wechat.zoro.build)申请邀请码后即可注册登录。
